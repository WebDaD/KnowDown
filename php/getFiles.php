...
<?php function findThings($folder){
	...
	foreach (scandir($folder) as $result) {
		...
		if(strpos($result,".")===false){ //directory
			$path = str_replace($FILE_PATH,"",$folder.$result);
			
			$folders .= '<li><a data-folder="'.$path.'/" class="tofolder"><img src="img/folder.png" style="margin-right:10px;"/>'.$result.'</a>';
		...
		if(strpos($result,".md")!==false){ //files
			...
			$files .= '<li><a data-filename="'.$path.'" class="tofile"><img src="img/document.png" style="margin-right:10px;"/>'.$display.'</a></li>'; 
		}
	}
